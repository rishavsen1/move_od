...
class Sg_combs:
    ...
    def __init__(self, county_cbg, output_path, ms_enabled, timedelta, time_start, time_end, start_date, end_date) -> None:
    ...
    def generate_OD(self, day, sg, county_cbg, res_build, com_build, ms_build, ms_enabled, time_slot1):       
        ...
        for idx, movement in sg.iterrows():
            ...
            if res.empty:
                if ms_enabled:
                    res = ms_build[ms_build.GEOID == movement.home_cbg].sample(n=int(movement.visits), random_state=42, replace=True).reset_index(drop=True)                       
                if res.empty:
            ...
            if com.empty:
                if ms_enabled:
                    com = ms_build[ms_build.GEOID == movement.poi_cbg].sample(n=int(movement.visits), random_state=42, replace=True).reset_index(drop=True)
                if com.empty:
    ...
    def main(self, county_cbg, res_build, com_build, ms_build, sg, cpus):
        ...
        while (day_count>0):
            ...
            for proc in range(num_processes):
                process = multiprocessing.Process(target=Sg_combs.generate_OD, args=(self,\
                                                day_sub[proc], sg[sg.date == day_sub[proc]],\
                                                county_cbg, res_build, com_build, ms_build, time_slot1))
                process.start()
