...
class Sg_combs:
    ...
    def generate_combs(self):
        ...
        sg = pd.read_csv(f'{self.data_path}/county_sg_first_chunk_of_five.csv', index_col = [0], dtype={'frequency': np.float64, 'home_cbg': str, 'poi_cbg': str})
        sg['frequency'] = sg['frequency'].astype(float)

        #grouping home and work location movements to get total no of movements
        sg = sg.groupby(['home_cbg', 'poi_cbg']).agg(frequency=('frequency', sum), visits_monday=('visits_monday', sum),
                                                     visits_tuesday=('visits_tuesday', sum),
                                                     visits_wednesday=('visits_wednesday', sum), 
                                                     visits_thursday=('visits_thursday', sum), 
                                                     visits_friday=('visits_friday', sum), 
                                                     visits_saturday=('visits_saturday', sum), 
                                                     visits_sunday=('visits_sunday', sum), 
                                                     median_dwell=('median_dwell', np.mean) ).reset_index()
