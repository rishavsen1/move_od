...
class Sg_combs:
    ...
    def __init__(self, county_cbg, data_path, ms_enabled, timedelta, time_start, time_end, start_date, end_date) -> None:
    ...
    def generate_combs(self):
        ...
        self.county_cbg['intpt'] = self.county_cbg[['INTPTLAT', 'INTPTLON']].apply(lambda p: Sg_combs.intpt_func(p), axis=1)
        self.county_cbg = gpd.GeoDataFrame(self.county_cbg, geometry=gpd.GeoSeries.from_wkt(self.county_cbg.geometry))
        ...
        self.res_build = pd.read_csv(f'{self.data_path}/county_residential_buildings.csv', index_col=[0])
        self.res_build = gpd.GeoDataFrame(self.res_build, geometry=gpd.GeoSeries.from_wkt(self.res_build.geometry))
        ...
        self.com_build = pd.read_csv(f'{self.data_path}/county_work_loc_poi_com_civ.csv', index_col=[0])
        self.com_build = gpd.GeoDataFrame(self.com_build, geometry=gpd.GeoSeries.from_wkt(self.com_build.geometry))
        ...
        self.ms_build = gpd.GeoDataFrame(self.ms_build, geometry=gpd.GeoSeries.from_wkt(self.ms_build.geo_centers))
        self.ms_build.GEOID = self.ms_build.GEOID.astype(str)
        self.ms_build['location'] = self.ms_build.geometry.apply(lambda p: [p.y, p.x])  

        self.time_slot1 = [datetime.strptime(dt.strftime('%H:%M'), '%H:%M') for dt in 
            Sg_combs.datetime_range(datetime(2016, 9, 1, 7, 0), datetime(2016, 9, 1, 18, 0), 
    ...
    def generate_OD(self, sg, days):       
        for day in days:
            print(f'Generating OD for {day}')
            prob_matrix_sg = gpd.GeoDataFrame()

            for idx, movement in sg[sg['date'] == day].iterrows():

                res = self.res_build[self.res_build.GEOID == movement.home_cbg].reset_index(drop=True)
                if res.empty:
                    if self.ms_enabled:
                        res = self.ms_build[self.ms_build.GEOID == movement.home_cbg].sample(n=int(movement.visits), random_state=42, replace=True).reset_index(drop=True)                       
                    if res.empty:
                        res = self.county_cbg[self.county_cbg.GEOID == movement.home_cbg].reset_index(drop=True)

                com = self.com_build[self.com_build.GEOID == movement.poi_cbg].reset_index(drop=True)
                if com.empty:
                    if self.ms_enabled:
                        com = self.ms_build[self.ms_build.GEOID == movement.poi_cbg].sample(n=int(movement.visits), random_state=42, replace=True).reset_index(drop=True)
                    if com.empty:
                        com = self.county_cbg[self.county_cbg.GEOID == movement.poi_cbg].reset_index(drop=True)
    ...
    def main(self):
        ...
        # if days> os.cpu_count():


        # for process in range(num_processes):
        #     process = multiprocessing.Process(target=Sg_combs.generate_OD, args=(sg, day))
